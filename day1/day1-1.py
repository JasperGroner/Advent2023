...
def get_line(string):
    ...
    for idx, number in enumerate(WRITTEN_NUMBERS):
        left_location = string.find(number)
        if left_location != -1 and (first_loc is None or left_location < first_loc):
            first_loc = left_location
            first = str(idx + 1)
        right_location = string.rfind(number)
        if right_location != -1 and (last_loc is None or right_location > last_loc):
            last_loc = right_location
            last = str(idx + 1)
...
class Tester(unittest.TestCase):
    def test_several(self):
        self.assertEqual(29, get_line("two1nine"))
        self.assertEqual(83, get_line("eightthree"))

testerOne = Tester()
testerOne.test_several()

print(get_sum("""two1nine
eightwothree
abcone2threexyz
xtwone3four
4nineeightseven2
