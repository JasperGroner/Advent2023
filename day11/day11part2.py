...
def adjust_galaxy_digest(galaxy_digest, empty_rows, empty_columns):
    ...
    # subtract one from the number of expansion times because default map alrady
    # has one
    MILLION_MINUS_ONE = 999999
    for galaxy in galaxy_digest:
        for idx, col in enumerate(empty_columns):
            if col > galaxy[0]:
                galaxy[0] += idx * MILLION_MINUS_ONE
                break
            if idx == len(empty_columns) - 1:
                galaxy[0] += (idx + 1) * MILLION_MINUS_ONE
        for idx, row in enumerate(empty_rows):
            if row > galaxy[1]:
                galaxy[1] += idx * MILLION_MINUS_ONE
                break
            if idx == len(empty_rows) - 1:
                galaxy[1] += (idx + 1) * MILLION_MINUS_ONE
